...
* [`src/scheduler/`](src/scheduler/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/scheduler/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `scheduler` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/scheduler/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns scheduler.example
  (:require [scheduler.config :as config])
